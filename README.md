...
```bash
cargo install rungbot          # the ladder. No keys, cannot trade.
cargo install rungbot-exec     # optional: live execution.
```
...
`plan` changes nothing without `--save`. All output takes `--json`.
...
168 tests, fully offline: `RUNGBOT_OFFLINE=1` makes every network layer refuse, so a test
that forgets to inject data fails instead of hitting an exchange. CI runs Linux, macOS and
Windows, plus clippy, rustfmt, the wasm32 build and an end-to-end smoke test.

`tests/golden/ladder_scenario.json` is a cross-language contract, generated by the
